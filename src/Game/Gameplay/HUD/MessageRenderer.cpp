...
MessageRenderer::MessageRenderer(float _height, const Color& _title_color,
		...
		health_bar(
				Rect<float>(SPACES * 3 + HEART_ICON_WIDTH + SPACES * 2, 16, 62,
						16), oglWrapper::RED, MAX_LIVES, Control::VERTICAL),

		heart_anim(3),

		score(0, 16, Body::NONE, nullptr, SCORE_ICON_WIDTH),
		score_bar(
				Rect<float>(health_bar.x + health_bar.w + SPACES * 8, 16, 62,
						16), oglWrapper::GREEN, max_score, Control::VERTICAL),

		retry_hud(
				Rect<float>(screen_bounds.x - 22 - SPACES * 2, 16, 16,
						score_bar.y + score_bar.h), "Od nowa", nullptr, false,
				this),

		game_over(oglWrapper::WHITE, "Game over", GLUT_BITMAP_HELVETICA_18, 18),
		//
		...
		controls_tooltip(oglWrapper::WHITE, "Sterowanie:",
				GLUT_BITMAP_HELVETICA_12, 12),

		/** Tooltip poziomu przjescia gry */
		game_progress_tooltip(oglWrapper::WHITE, "Ukonczono w:",
				GLUT_BITMAP_HELVETICA_12, 12),

		game_progress(oglWrapper::YELLOW, "", GLUT_BITMAP_HELVETICA_18, 18),

		/** Tooltip ilosc punktow */
		game_score_tooltip(oglWrapper::WHITE, "Lacznie punktow:",
				GLUT_BITMAP_HELVETICA_12, 12),
...
void MessageRenderer::create() {
	...
	retry_game = new Button(
			Rect<float>(screen_bounds.x / 2 - 10 - 100, screen_bounds.y - 45,
					100, 35), "Od nowa", nullptr, this);
	retry_game->putCallback(Event::MOUSE_RELEASED, this);

	return_to_menu = new Button(
			Rect<float>(screen_bounds.x / 2 + 10, screen_bounds.y - 45, 100,
					35), "Do menu");
	...
	arrows_icons[pEngine::UP] =
			new IrregularPlatform(28, 30, Body::NONE,
					wsad ? readShape("strzalka_w.txt", "strzalka_gora", 0.f) : readShape(
									"strzalka_gora.txt", "strzalka_gora", 0.f),
					18);

	arrows_icons[pEngine::LEFT] =
			new IrregularPlatform(arrows_icons[pEngine::UP]->x - 19,
					arrows_icons[pEngine::UP]->y + 21, Body::NONE,
					wsad ? readShape("strzalka_a.txt", "strzalka_lewo", 0.f) : readShape(
									"strzalka_gora.txt", "strzalka_lewo",
									-90.f), 18);
	...
	arrows_icons[pEngine::RIGHT] =
			new IrregularPlatform(arrows_icons[pEngine::UP]->x + 19,
					...
					wsad ? readShape("strzalka_d.txt", "strzalka_prawo", 0.f) : readShape(
									"strzalka_gora.txt", "strzalka_prawo",
									90.f), 18);
}
...
void MessageRenderer::drawBorder(Window* _window) {
	if (screen == INTRO_SCREEN) {
		...
		oglWrapper::drawFillRect(SPACES, screen_bounds.y - height + SPACES,
				screen_bounds.x - SPACES * 2, height, oglWrapper::BLACK);
		...
		oglWrapper::drawFillRect(SPACES * 2, screen_bounds.y - height,
	...
	if (hud_temperature != _actual_temperature) {
		...
		heart.setShape(
				ResourceFactory::getInstance(nullptr).getTexture(
						ResourceFactory::HEALTH, pEngine::NONE));
		...
		score.setShape(
				ResourceFactory::getInstance(nullptr).getTexture(
						ResourceFactory::SCORE, pEngine::NONE));

		score.fitToWidth(hud_temperature == ResourceFactory::ICY ?
...
void MessageRenderer::drawDeathScreen(Window*) {
	game_over.printText(screen_bounds.x / 2 - game_over.getScreenLength() / 2,
			80);
	//
	retry_game->drawObject(nullptr);
	return_to_menu->drawObject(nullptr);
	//
	glText t(oglWrapper::YELLOW, "[Wcisnij SPACJE by sprobowac ponownie]",
			GLUT_BITMAP_HELVETICA_18, 18);
	t.printText(screen_bounds.x / 2 - t.getScreenLength() / 2, 110);
}
...
void MessageRenderer::drawGameInfo(Window*) {
	...
	game_progress.printText(
			screen_bounds.x / 2 - game_progress.getScreenLength() / 2, 46);

	/** ILOSC PUNKTOW */
	game_score_tooltip.printText(
			screen_bounds.x - game_score_tooltip.getScreenLength() - 25, 22);

	string score =
			!hero ? "0" : Convert::toString<usint>(
	...
	game_score.printText(
			screen_bounds.x - game_score_tooltip.getScreenLength() / 2
					- game_score.getScreenLength() / 2 - 25, 46);
}
